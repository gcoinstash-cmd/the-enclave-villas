...
export function Concierge({ properties, triggerToast }: ConciergeProps) {
  ...
  const handleSendMessage = async (customText?: string) => {
    ...
    try {
      ...
      // Intelligent architectural advisor fallback
      const lower = textToSend.toLowerCase();
      let responseText = "";
      if (lower.includes("brutalist") || lower.includes("pavilion") || lower.includes("concrete")) {
        responseText = "Across our active portfolio, **The Obsidian Pavilion** ($8,450,000 in Big Sur) and **The Brutalist Sanctuary** ($6,900,000 in Sintra) are our primary concrete landmarks. Both feature cast basalt foundations, charred cedar facades, and integrated floor-to-ceiling thermal glazing. Would you like me to prepare a private showing dossier?";
      } else if (lower.includes("6m") || lower.includes("price") || lower.includes("recommend") || lower.includes("cost")) {
        responseText = "For portfolios under $6,000,000, we recommend **The Minimalist Atrium House** ($4,200,000 in Kyoto). It spans 3,100 sq ft organized around a private Zen maple court with board-formed concrete and radiant heated limestone flooring.";
      } else if (lower.includes("security") || lower.includes("vault") || lower.includes("lock") || lower.includes("private")) {
        responseText = "Absolute privacy is anchored by **The Brutalist Sanctuary** (Sintra, Portugal) which features a 10-acre private pine forest perimeter and a military-grade biometric security vault, alongside a cantilevered wellness capsule.";
      } else {
        responseText = `Our private collection currently features ${properties.length} certified architectural masterworks:
• **The Obsidian Pavilion** — Big Sur ($8.45M)
• **Minimalist Atrium House** — Kyoto ($4.2M)
• **The Champagne Penthouse** — Tribeca ($12.5M)
• **The Brutalist Sanctuary** — Sintra ($6.9M)

Every acquisition includes deed verification, private escrow handling, and confidential VIP access. How may I further refine your criteria?`;
      }
