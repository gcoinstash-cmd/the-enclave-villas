...
import { Compass, HelpCircle, UserCheck, Eye, Sparkles, Send, X, Calendar, Key, FilePlus } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    // Direct Admin route check
    const path = window.location.pathname.toLowerCase();
    const hash = window.location.hash.toLowerCase();
    const search = window.location.search.toLowerCase();
    if (path.includes('admin') || hash.includes('admin') || search.includes('admin')) {
      setActiveTab('dashboard');
      setSandboxRole('broker');
      setIsAdminUnlocked(true);
      setTimeout(() => triggerToast('⚡ Admin Passkey Activated: Managing Partner Access Granted'), 300);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#050505] text-neutral-200">
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            ...
            <div className="flex items-center gap-1 md:hidden">
              <button
                ...
                className="p-2 text-gold-400 hover:text-[#E5D3B3]"
                title="Admin Passkey"
              >
            ...
            {/* 1-Click Ghost Factory Passkey Door */}
            <button
              onClick={() => setIsAdminPassOpen(true)}
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 bg-[#14120F] hover:bg-[#201C15] border border-gold-500/30 hover:border-gold-500/60 rounded-md text-xs font-semibold tracking-wider font-mono uppercase tracking-[0.2em] text-[#E5D3B3] transition-all duration-300 cursor-pointer shadow-[0_0_15px_rgba(176,158,81,0.15)]"
            ...
            <button
              ...
              className="px-3.5 py-1.5 sm:px-4 sm:py-2 bg-transparent hover:bg-white/[0.02] border border-white/10 hover:border-gold-500/25 rounded-md text-xs font-semibold tracking-wider font-mono uppercase tracking-[0.2em] text-neutral-400 hover:text-[#E5D3B3] transition-all duration-300 cursor-pointer"
      ...
      {/* C. 1-Click Ghost Factory Admin Passkey Gate */}
      <AnimatePresence>
        {isAdminPassOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-[#0A0A0B] border border-gold-500/30 rounded-xl max-w-md w-full p-6 sm:p-8 space-y-6 text-left shadow-[0_0_50px_rgba(176,158,81,0.15)] relative"
            >
              <button
              ...
              <div className="space-y-2">
                <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-gold-500/10 border border-gold-500/30 text-[9px] font-mono text-gold-400 uppercase tracking-widest">
                  <Sparkles className="w-3 h-3" />
                  <span>Ghost Factory™ Passkey Door</span>
                </div>
                <h3 className="font-display text-2xl font-light text-white tracking-tight">
                  The Enclave Control Room
                </h3>
                <p className="text-xs text-neutral-400 leading-relaxed font-light">
                  Enter the managing partner passkey to unlock the private CRM, villa listings engine, maintenance queue, and client transaction records.
                </p>
              </div>

              <div className="p-3 bg-neutral-950/80 border border-gold-500/20 rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold tracking-wider font-mono uppercase tracking-wider text-neutral-400">Demo Passcode:</span>
                  <code className="text-xs font-mono font-bold text-gold-400 bg-black/60 px-2 py-0.5 rounded border border-gold-500/30">enclave2026</code>
                </div>
                <button
                  type="button"
                  onClick={() => setAdminPassInput('enclave2026')}
                  className="mt-2.5 w-full py-2 bg-gold-500/10 hover:bg-gold-500/20 border border-gold-500/30 hover:border-gold-500/50 rounded text-xs font-semibold tracking-wider font-mono uppercase tracking-widest text-[#E5D3B3] transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  if (adminPassInput.trim() === 'enclave2026') {
                    setIsAdminUnlocked(true);
                    setSandboxRole('broker');
                    setActiveTab('dashboard');
                    setIsAdminPassOpen(false);
                    setAdminPassInput('');
                    triggerToast('👑 Managing Partner Access Granted (Cheat Code Verified)');
                  } else {
                    triggerToast('❌ Invalid Passkey. Use demo passcode: enclave2026');
                  }
                }}
                className="space-y-4"
              >
                <div>
                  <label className="block text-sm font-semibold tracking-wider font-mono uppercase tracking-widest text-neutral-400 mb-1.5">
                    Security Passcode
                  </label>
                  <input
                    type="password"
                    ...
                    placeholder="Enter enclave2026"
                    className="w-full px-4 py-3 bg-neutral-950 border border-neutral-800 focus:border-gold-500 rounded-md text-sm font-mono text-white outline-none transition-colors"
                    autoFocus
                  />
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setIsAdminPassOpen(false)}
                    className="flex-1 py-3 px-4 bg-neutral-900 hover:bg-neutral-800 text-neutral-400 hover:text-white border border-neutral-800 text-xs font-mono uppercase tracking-widest rounded-md transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-3 px-4 bg-[#B09E51] hover:bg-[#C1B369] text-black font-semibold text-base font-semibold min-h-[44px] font-mono uppercase tracking-widest rounded-md transition-all shadow-lg shadow-gold-500/20"
                  >
                    Unlock Board
                  </button>
                </div>
